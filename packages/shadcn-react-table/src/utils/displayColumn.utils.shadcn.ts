import type {
	...
	SRT_DisplayColumnIds,
	SRT_Localization,
	SRT_RowData,
	SRT_StatefulTableOptions,
} from '../types-SRT'
import { getAllLeafColumnDefs, getColumnId } from './column.utils.shadcn'
...
export const getLeadingDisplayColumnIds = <TData extends SRT_RowData>(tableOptions: SRT_StatefulTableOptions<TData>) =>
	[
		showRowPinningColumn(tableOptions) && 'srt-row-pin',
		showRowDragColumn(tableOptions) && 'srt-row-drag',
		tableOptions.positionActionsColumn === 'first' && showRowActionsColumn(tableOptions) && 'srt-row-actions',
		tableOptions.positionExpandColumn === 'first' && showRowExpandColumn(tableOptions) && 'srt-row-expand',
...
export const getTrailingDisplayColumnIds = <TData extends SRT_RowData>(tableOptions: SRT_StatefulTableOptions<TData>) =>
	[
		tableOptions.positionActionsColumn === 'last' && showRowActionsColumn(tableOptions) && 'srt-row-actions',
		tableOptions.positionExpandColumn === 'last' && showRowExpandColumn(tableOptions) && 'srt-row-expand',
