import type { SRT_Header, SRT_Cell, SRT_RowData, SRT_TableInstance } from '../types-SRT'
import { getSRT_RowSelectionHandler, getSRT_SelectAllHandler } from './row.utils.shadcn'
...
export const cellKeyboardShortcuts = <TData extends SRT_RowData = SRT_RowData>({
	...
	if (cellValue && isWinCtrlMacMeta(event) && event.key === 'c') {
		...
		if (cell?.column?.id === 'srt-row-select') {
		...
		} else if (header?.column?.id === 'srt-row-select' && table.options.enableSelectAll) {
			event.preventDefault()
			getSRT_SelectAllHandler({
				table,
			})(event as any)
		} else if (
			cell?.column?.id === 'srt-row-expand' &&
			(cell.row.getCanExpand() || table.options.renderDetailPanel?.({ row: cell.row, table }))
		) {
			event.preventDefault()
			cell.row.toggleExpanded()
		} else if (header?.column?.id === 'srt-row-expand' && table.options.enableExpandAll) {
			event.preventDefault()
			table.toggleAllRowsExpanded()
		} else if (cell?.column.id === 'srt-row-pin') {
			event.preventDefault()
		...
		['ArrowRight', 'ArrowLeft', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown'].includes(event.key)
	) {
		event.preventDefault()
		...
		const currentIndex = Number.parseInt(currentCell.getAttribute('data-index') || '0')
		...
		const findBottomTopCell = (columnIndex: number, edge: 'b' | 't') => {
			const row =
				edge === 't' ? tableElement?.querySelector('tr') : tableElement?.lastElementChild?.lastElementChild
			const rowCells = Array.from(row?.children || [])
