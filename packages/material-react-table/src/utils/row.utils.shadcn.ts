...
export const getSRT_Rows = <TData extends SRT_RowData>(
	...
	if (positionCreatingRow !== undefined && creatingRow && createDisplayMode === 'row') {
		const creatingRowIndex = !Number.isNaN(+positionCreatingRow)
			? +positionCreatingRow
...
export const getCanRankRows = <TData extends SRT_RowData>(table: SRT_TableInstance<TData>) => {
	const {
		getState,
...
export const getIsRankingRows = <TData extends SRT_RowData>(table: SRT_TableInstance<TData>) => {
